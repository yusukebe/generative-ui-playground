...
export async function preparePlan(
  env: CloudflareBindings,
  params: PlanParams
): Promise<{ weather: Weather | null; restaurants: Restaurant[] }> {
  ...
  // 〆ラーメン候補を末尾に合流 (genre='家系ラーメン' で区別)
  return { weather, restaurants: [...izakaya, ...ramen] }
}
...
function planContext(params: PlanParams, weather: Weather | null, restaurants: Restaurant[]): string {
  const w = weather
...
プラン構成: 1軒目・2軒目は居酒屋系から、最後の「〆」は家系ラーメンの店を1つ使うこと。`
}
...
export function streamBand(
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      const ctx = planContext(params, weather, restaurants)
