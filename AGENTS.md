...
- **4 パターン** (ラベルは Static/Declarative/Open-Ended/Dynamic。band id は controlled/declarative/open-ended/dynamic):
  - **Static = 1 フェーズ**: AI は 1 回の `streamText` でデータツール(`get_weather`/`get_last_train`/`search_restaurants`/`get_ramen`)を**呼ぶだけ**(必須でなく LLM が選ぶ=出る部品の集合は可変)。呼ばれたツール出力を `switch(tool)` で固定部品へ(`StaticView.tsx`)。**表示順はホスト固定(天気→店→〆→終電)** ※参考実装は LLM 呼び出し順だが「〆が最初」等で不自然なので固定にした(仕組みは参考実装の「tool-output → component」、順だけ host 固定)。title はホスト固定テンプレ(クライアント即描画)。「ツール」ソースは AI の生のツールコール列(=呼び出し順)、メトリクス字数もそのツール列長。
